...
RSpec.describe Api::V1::UsersController, type: :controller do
    ...
    describe "PUT update" do
        context "with valid attributes" do
            before do
                @new_user = build(:user)
                @new_user.admin!
                controller.request.env['HTTP_AUTHORIZATION'] = ActionController::HttpAuthentication::Token.encode_credentials(@new_user.auth_token)
                put :update, id: @new_user.id, user: { name: @new_user.name, email: @new_user.email, password: @new_user.password, role: "admin" }
            end
    ...
    describe "POST create" do
        context "with valid attributes" do
            before do
                @new_user1 = build(:user)
                @new_user1.admin!
                @new_user = build(:user)
                controller.request.env['HTTP_AUTHORIZATION'] = ActionController::HttpAuthentication::Token.encode_credentials(@new_user1.auth_token)
